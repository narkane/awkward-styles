...
var line4;
 	$(document).ready(function() {
		...
		canvas2 = new fabric.Canvas('tcanvas2', {
		  ...
		  canvas3 = new fabric.Canvas('tcanvas3', {
			hoverCursor: 'pointer',
			selection: true,
			selectionBorderColor:'blue'
		  });

		  canvas4 = new fabric.Canvas('tcanvas4', {
			hoverCursor: 'pointer',
			selection: true,
			selectionBorderColor:'blue'
		  });
		...
		canvas2.findTarget = (function(originalFn) {
		  ...
		  canvas3.findTarget = (function(originalFn) {
			return function() {
			  var target = originalFn.apply(this, arguments);
			  if (target) {
				if (this._hoveredTarget !== target) {
		  ...
		  canvas4.findTarget = (function(originalFn) {
			return function() {
			  var target = originalFn.apply(this, arguments);
			  if (target) {
				if (this._hoveredTarget !== target) {
		...
		canvas3.on('object:over', function(e) {		
			//e.target.setFill('red');
			//canvas.renderAll();
		});
		  
		canvas3.on('object:out', function(e) {		
			//e.target.setFill('green');
			//canvas.renderAll();
		});

		canvas4.on('object:over', function(e) {		
			//e.target.setFill('red');
			//canvas.renderAll();
		});
		  
		canvas4.on('object:out', function(e) {		
			//e.target.setFill('green');
			//canvas.renderAll();
		});
		...
		document.getElementById('add-text').onclick = function() {
			...
			canvas3.add(textSample);
			canvas4.add(textSample);	
			canvas.item(canvas.item.length-1).hasRotatingPoint = true;  
			canvas2.item(canvas.item.length-1).hasRotatingPoint = true;    
	  ...
	  document.getElementById('remove-selected').onclick = function() {		  
		    ...
		    if (activeObject) {
			  ...
			  canvas3.remove(activeObject);
			  canvas4.remove(activeObject);
		      $("#text-string").val("");
		    }
		    else if (activeGroup) {
		      ...
		      objectsInGroup.forEach(function(object) {
				...
				canvas3.remove(object);
				canvas4.remove(object);
		      });
		    }
	  };

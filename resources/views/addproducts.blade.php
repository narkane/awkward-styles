                ...
                <div class="tab-content" id="progressbar-tabContent">
                    <div class="tab-pane show active" id="progressbar-1" role="tabpanel" aria-labelledby="progressbar-1-tab">
                        ...
                        <div class="col-md-12 pricing-product-list">
                                    ...
                                    <div class="row">
                        @if(count($artworks)>0)
                        @foreach($artworks['properties'] as $artwork)
                        <div class="col-md-3">
                                            <div class="pricing-product">
                                                <label class="checkbox-container">
                                                                    <input type="checkbox" 
                                        value="{{ $artwork['mediaId']['full_url'] }}" class="radio" artworkID="{{ $artwork['mediaId']['full_url'] }}" name="artwork">
                                                                    <span class="checkmark"></span>
                                                              <div class="product-wrap">
                                                                <div class="product-image">
                                                                <img src="{{ $artwork['mediaId']['full_url'] }}" width="100%">
                                                                </div>  
                                                                <div class="product-information">
                                                                    <h6 class="product-title">{{ $artwork['artName'] }}</h6>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12 pricing-product-list">
                                    ...
                                    <input type="hidden" id="storeproducts" name="selectedproducts">
                                        @if(count($styles)>0)
                                        @foreach($styles['properties'] as $style)
                                        <div class="col-md-3">
                                            <div class="pricing-product">
                                                <label class="checkbox-container">
                                                                    <input type="checkbox" onclick="selectProduct({{ $style['id'] }})" id="{{ $style['id'] }}">
                                                                    <span class="checkmark"></span>
                                                              <div class="product-wrap">
...
<div id="myModal{{ $style['id'] }}" class="modal" role="dialog">
 ...
 <script defer src="{{url('/')}}/mockupgen_assets/js/html2canvas.js"></script>
  <script defer src="{{url('/')}}/mockupgen_assets/js/art_canvas2.js"></script>
    ...
    margin: 1.75rem auto;
                    ...
                    console.log("addP");
                        ...
                        html2canvas(document.getElementById("print_canvas"+id)).then(canvas => {
                            ...
                            console.log(canvas,"id",id);
                             $('#style_{{ $style['id'] }}').attr('src',canvas);                             
                        });
     ...
     @endif
        ...
        $(document).ready(function() {
                $("#myModal{{ $style['id'] }}").on('shown.bs.modal', function (e) {
                  // do cool stuff here all day… no need to change bootstrap
                                console.log("modeal loaded");
                        myFunction( $("input[name='artwork']:checked").val(),ArtItemType.IMAGE,"{{ $style['id'] }}");
                        setShirtImage( $("#style_{{ $style['id'] }}_1").val() , "{{ $style['id'] }}");
                                        console.log("asfadsf");
                });
   ...
   <!-- Modal content-->
    <div class="modal-content">
     ...
     <div id="print_canvas{{ $style['id'] }}"  style="width:560px; height:600px; margin:0 auto; border:1px solid black; position: relative;">
                  <input type="hidden" id="style_{{ $style['id'] }}_1">
                  <div id="container_shirt{{ $style['id'] }}" style="width:560px; height:600px;"></div> 
                  <div id="canvas_parrent{{ $style['id'] }}" style="
                  ...
                  margin:0 auto; border:1px solid #3c3c3c;position: absolute;z-index: 9999;transform:translate(-50%,-50%);">
                    <div id="container{{ $style['id'] }}"></div>
                  </div>
                </div>
      </div>
      <div class="modal-footer">
        <button type="button" onClick="showImagetoMainScreen('{{ $style['id'] }}');" class="btn btn-primary">Save changes</button>
                                                                ...
                                                                <div class="product-information">
                                                                    <h6 class="product-title">{{ $style['label'] }}</h6>
                                ...
                                <div class="col-md-3">
                                    ...
                                    <div class="refine-section">
                                        ...
                                        <div class="refine-filters">
                                            ...
                                            <ul>
                                            @if(count($brands)>0)
                                            @foreach($brands['properties'] as $brand)
                                            <li>
                                            <label class="checkbox-container">{{ $brand['brandName'] }}
                                            <input onclick="selectBrand({{ $brand['id'] }})"  id="{{ $brand['id'] }}" type="checkbox">
                                            <span class="checkmark"></span>
                                            </label>
                                            </li>
                                            @endforeach   
...
console.log(id);
                            ...
                            var baseUrl = "{{env('API_URL')}}api/artworkProduct/save";
...
function selectProductTab(){
                            ...
                            console.log(query);
                            var getUrl = window.location;
                            var baseUrl = "{{env('API_URL')}}api/product/getProductsByCriteria";
                         ...
                         success: function(productsType){ 
                            ...
                            if(productsType.operationCode == 200){
                             ...
                             /*   var baseUrl = getUrl .protocol + "//" + getUrl.host + "/" + getUrl.pathname.split('/')[0]+':8080'+'/api/media/mergeImageWithArtWork?imageId='+image[0]+'&artWorkImageId='+$("input[name='artwork']:checked").val();*/
                              ...
                              var baseUrl = "{{env('API_URL')}}api/media/getById/"+image[0];
                            ...
                            if(imagedata.operationCode == 200){ 
                                ...
                                var finalImage = "";  
                                var baseUrl = "{{env('API_URL')}}api/media/mergeImagePathsWithArtWork/";
        ...
        $(document).ready(function() {
            ...
            if($('#progressbar-2-tab').attr('class') == 'active show'){
                                    ...
                                    @if(count($styles)>0)
                                        @foreach($styles['properties'] as $style)
                                        var getUrl = window.location;
                            var baseUrl = "{{env('API_URL')}}api/media/getById/{{ $style['image'] }}";
                            var url = baseUrl.replace('/:', ':')
                            var url = url;//form.attr('action');  
                            console.log(url);          
                            $.ajax({url: url, contentType: 'application/json', dataType: 'json', headers: {
             ...
             var baseUrl = "{{env('API_URL')}}api/media/mergeImagePathsWithArtWork/";
                            ...
                            $('#style_{{ $style['id'] }}_1').val(imagedata.properties.full_url);

                            $('#style_{{ $style['id'] }}_art').attr('src',$("input[name='artwork']:checked").val());
                           ...
                           // console.log(url);
                            ...
                            if(finalimagedata.operationCode == 200){
                               ...
                               $('#style_{{ $style['id'] }}').attr('src',finalimagedata.properties.full_url);

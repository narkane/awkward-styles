...
/**
 * Factory for service JQAdm client
 *
 * @package Admin
 * @subpackage JQAdm
 */
	...
	implements \Aimeos\Admin\JQAdm\Common\Factory\Iface
{
	...
	public static function createClient( \Aimeos\MShop\Context\Item\Iface $context, $name = null )
	{
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'admin/jqadm/templates/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? '\\Aimeos\\Admin\\JQAdm\\Templates\\' . $name : '<not a string>';
		...
		$classname = '\\Aimeos\\Admin\\JQAdm\\Templates\\' . $name;

		$client = self::createClientBase( $context, $classname, $iface );

		return self::addClientDecorators( $context, $client, 'templates' );
	}

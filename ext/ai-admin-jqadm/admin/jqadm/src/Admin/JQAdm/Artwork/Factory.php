	...
	implements \Aimeos\Admin\JQAdm\Common\Factory\Iface
{
	...
	public static function createClient( \Aimeos\MShop\Context\Item\Iface $context, $name = null )
	{
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'admin/jqadm/artwork/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? '\\Aimeos\\Admin\\JQAdm\\Artwork\\' . $name : '<not a string>';
		...
		$classname = '\\Aimeos\\Admin\\JQAdm\\Artwork\\' . $name;

		$client = self::createClientBase( $context, $classname, $iface );

		return self::addClientDecorators( $context, $client, 'artwork' );
	}

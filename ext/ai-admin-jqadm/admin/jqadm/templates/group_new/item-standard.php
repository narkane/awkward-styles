...
<form class="item item-group form-horizontal" method="POST" enctype="multipart/form-data" action="<?= $enc->attr( $this->url( $target, $cntl, $action, $params, [], $config ) ); ?>">
	<input id="item-id" type="hidden" name="<?= $enc->attr( $this->formparam( array( 'item', 'customer.group_new.id' ) ) ); ?>" value="<?= $enc->attr( $this->get( 'itemData/customer.group_new.id' ) ); ?>" />
	...
	<nav class="main-navbar">
		<span class="navbar-brand">
			...
			<?= $enc->html( $this->get( 'itemData/customer.group_new.id' ) ); ?> -
			<?= $enc->html( $this->get( 'itemData/customer.group_new.label', $this->translate( 'admin', 'New' ) ) ) ?>
			<span class="navbar-secondary">(<?= $enc->html( $this->site()->match( $this->get( 'itemData/customer.group_new.siteid' ) ) ); ?>)</span>
		</span>
		...
		<div class="col-md-3 item-navbar">
			...
			<div class="item-meta text-muted">
				<small>
					...
					<span class="meta-value"><?= $enc->html( $this->get( 'itemData/customer.group_new.mtime' ) ); ?></span>
				</small>
				<small>
					<?= $enc->html( $this->translate( 'admin', 'Created' ) ); ?>:
					<span class="meta-value"><?= $enc->html( $this->get( 'itemData/customer.group_new.ctime' ) ); ?></span>
				</small>
				<small>
					<?= $enc->html( $this->translate( 'admin', 'Editor' ) ); ?>:
					<span class="meta-value"><?= $enc->html( $this->get( 'itemData/customer.group_new.editor' ) ); ?></span>
				</small>
			</div>
		</div>

		<div class="col-md-9 item-content tab-content">
			<?php $readonly = ( $this->access( ['admin', 'super'] ) === false ? $this->site()->readonly( $this->get( 'itemData/customer.group_new.siteid' ) ) : '' ); ?>
					...
					<div class="form-group row mandatory">
						...
						<div class="col-sm-8">
							<input class="form-control item-code" type="text" required="required" tabindex="1"
								name="<?= $enc->attr( $this->formparam( array( 'item', 'customer.group_new.code' ) ) ); ?>"
								placeholder="<?= $enc->attr( $this->translate( 'admin', 'Unique group code (required)' ) ); ?>"
								value="<?= $enc->attr( $this->get( 'itemData/customer.group_new.code' ) ); ?>"
								<?= $this->site()->readonly( $this->get( 'itemData/customer.group_new.siteid' ) ); ?> />
						</div>
					...
					<div class="form-group row mandatory">
						...
						<div class="col-sm-8">
							<input class="form-control item-label" type="text" required="required" tabindex="1"
								name="<?= $this->formparam( array( 'item', 'customer.group_new.label' ) ); ?>"
								placeholder="<?= $enc->attr( $this->translate( 'admin', 'Internal name (required)' ) ); ?>"
								value="<?= $enc->attr( $this->get( 'itemData/customer.group_new.label' ) ); ?>"
								<?= $this->site()->readonly( $this->get( 'itemData/customer.group_new.siteid' ) ); ?> />
						</div>
						<div class="col-sm-12 form-text text-muted help-text">

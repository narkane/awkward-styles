					...
					<ul class="select-list" data-index="<?= $index++; ?>" data-type="<?= $enc->attr( $code ); ?>">
						<?php foreach( $positions as $attrId => $position ) : ?>
							...
							<li class="select-entry">
								...
								/>
								<label class="select-label" for="option-<?= $enc->attr( $attrId ); ?>" style="background-color:<?= $enc->html( $attributeItems[$attrId]->getCode() ); ?>"><!--
									...
									--><span style="display:none"><?= $enc->html( $attributeItems[$attrId]->getName() ); ?></span><!--
								--></label>
							</li>

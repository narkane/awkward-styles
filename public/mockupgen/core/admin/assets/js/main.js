	...
	var invert = function(color) {
		...
		process_submit_upload = function() {
			...
			$('input.lumise-upload-helper-inp').each(function() {
				...
				if (el.val().indexOf('data:image/') === -1)
					return;
				...
				var id = $('input[name="id"]').val(),	
					submit_url = LumiseDesign.ajax+'&action=upload&task=media&nonce=LUMISE_ADMIN:'+LumiseDesign.nonce,
					boundary = "---------------------------7da24f2e50046",
					body = '--' + boundary + '\r\n'
				         + 'Content-Disposition: form-data; name="file";'
				         + 'filename="1.txt"\r\n'
				         + 'Content-type: plain/text\r\n\r\n'
				         + btoa(encodeURIComponent(el.val()))
				         + '\r\n'
				         + '--'+ boundary + '--';
				...
				$.ajax({
				    ...
				    success: function (res, status) {
					    ...
					    el.removeAttr('data-processing');
					    el.val(decodeURIComponent(res.success));
	...
	window.lumise = {
		...
		product: {		
				...
				trigger({
					...
					before_submit: function(e) {
						...
						*/
						...
						return lumise.product.upload_images_submit(stages, variations);
					...
					popup_click: function(e) {
						...
						switch (act) {
							...
							case 'samples' : 
								$('#lumise-uploaded-bases,button[data-act="samples"]').addClass('hidden');
								$('#lumise-sample-bases,button[data-act="uploaded"]').removeClass('hidden');
								e.preventDefault();
							break;
							
							case 'uploaded' : 
								$('#lumise-uploaded-bases,button[data-act="samples"]').removeClass('hidden');
				...
				trigger({
					...
					load_attributes: function(e) {
						...
						Object.keys(data).map(function(id) {
							...
							tmpl.find('textarea[data-name="values"]').val(attr.values);
					...
					refresh_variations: function(e) {
						...
						if (Object.keys(attrs).length === 0) {
							...
							vars.find('[data-act="add_variation"], a[data-act]').hide();
							vars.find('p[data-view="notice"]').show();
							
						} else {
							
							vars.find('[data-act="add_variation"], a[data-act]').show();
							vars.find('p[data-view="notice"]').hide();
							
							Object.keys(attrs).map(function(k) {
								var ops = '<option value="">Any '+esc_html(attrs[k].name)+'</option>';
								if (attrs[k].values !== undefined) {
									attrs[k].values.split("\n").map(function(at) {
										at = at.split('|');
										at[0] = at[0].trim();
										ops += '<option '+(
											current[k] !== undefined && current[k] == at[0] ? 'selected' : ''
										)+' value="'+esc_html(at[0])+'">'+esc_html(at[1] !== undefined ? at[1].trim() : at[0])+'</option>';
									});
								};
								...
								Object.keys(attrs).map(function(k) {
									...
									if (attrs[k].values !== undefined) {
										attrs[k].values.split("\n").map(function(at) {
											...
											at[0] = at[0].trim();
											ops += '<option '+(
												current[k] !== undefined && current[k] == at[0].trim() ? 'selected' : ''
											)+' value="'+esc_html(at[0])+'">'+esc_html(at[1] !== undefined ? at[1].trim() : at[0])+'</option>';
										});
									};
			...
			upload_images_submit: function(stages, variations) {
				...
				var submit_url = LumiseDesign.ajax+'&action=upload&task=stages&nonce=LUMISE_ADMIN:'+LumiseDesign.nonce,
					boundary = "---------------------------7da24f2e50046",
					body = '--' + boundary + '\r\n'
				         + 'Content-Disposition: form-data; name="file";'
				         + 'filename="file.txt"\r\n'
				...
				$.ajax({
				    ...
				    xhr: function() {
					    ...
					    xhr.upload.addEventListener("progress", function(evt){
					      if (evt.lengthComputable) {
					        ...
					        if (percentComplete === 1)
					        	txt = '<i class="fa fa-spin fa-refresh"></i> Submitting..';
					       	$('#lumise-product-form-submitting').html(txt);
					      }
					    }, false);
					    return xhr;
					},
				    success: function (res, status) {
					    ...
					    $('textarea#lumise-field-stages-inp').val(res[0]);
					    $('textarea#lumise-field-variations-inp').val(res[1]);
					    $('#lumise-product-form').off('submit').submit();
			...
			get_attributes: function(is_vari) {
				...
				$('#lumise-field-attributes-items>.lumise-att-layout-item').each(function() {
					...
					if (
						...
						var values = el.find('textarea[data-name="values"]').val().trim().split("\n");
						
						values.map(function(vl, i) {
							vl = vl.split('|');
							vl = vl[0].trim()+(vl[1] !== undefined ? ' | '+vl[1].trim() : '');
							values[i] = vl;
						});
					...
					if (type == 'product_color')
						color = el.find('textarea[data-name="values"]')
								  .val()
								  .split("\n")[0]
								  .split('|')[0].trim();
				});
			...
			get_stages: function(wrp) {
				...
				*/
				...
				wrp.find('.lumise_tab_nav li:not([data-add]) a').each(function() {
					...
					do {
						if (prtn.css('display') == 'none') {
							prtn.css({display: 'inline-block'});
							prtns.push(prtn.get(0));
						}
						prtn = prtn.parent();
					} while (prtn.get(0).tagName != 'BODY');
				...
				if (this.files && this.files[0]) {
			        ...
			        reader.onload = function (e) {
						...
						if (type == 'font') {
							...
							Modified by sravan, instead of decoding , directly passing result for image preview*/
						...
						if (attr('file-input')) {
							...
							if (attr('file-thumbn-width') || attr('file-thumbn-height')) {
								...
								if (type == 'design')
									/*modified by sravan, instead of scre, directly passing result*/
									data.thumbn = result;
								
								$(attr('file-input')).val(btoa(encodeURIComponent(JSON.stringify(data))));
							}

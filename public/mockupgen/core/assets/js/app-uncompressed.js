	...
	var lumise = {
		...
		objects : {
				...
				'mouse:up': function(opts) {
					...
					if (onbox !== null) {
						if (actv) {
							...
							return lumise.func.imagebox_select_file(onbox);
					};
			...
			lumise : {
				...
				'i-text' : function(ops, callback) {
					...
					ops.src = 'data:image/svg+xml;base64,'+btoa(lumise.func.buildText(ops));
				...
				'image' : function(ops, callback) {
					...
					) return lumise.func.replace_image(ops.src, active);
					...
					fabric.util.loadImage(ops.src, function(img) {
					    ...
					    if(img == null) {
					        ...
					        callback(null);
						   	...
						   	var src_data = lumise.func.check_upload_dimensions(img);
							...
							if (ops.evented === undefined) {
								if (ibx !== null && !do_replace) {
									...
									ops.width = ibx.width;
									ops.height = image.height*(ibx.width/image.width);
									ops.left = ibx.left;
									ops.top = ibx.top;
									
									if (ops.height < ibx.height) {
										ops.height = ibx.height;
										ops.width = image.width*(ops.height/image.height);
									};
		...
		design : {
			...
			events : function(){
					...
					general_status: function(e) {
						...
						switch (func) {
							...
							case 'cancel-design': 
								...
								lumise.func.set_url('my_design', null);
							...
							case 'save-design':
								console.log('save design-2067');
								lumise.func.export('designs');
								stage = lumise.stage();
				...
				$.ajax({
							url: 'savedesigns.php',
							method: 'POST',
							data: {'selectedimage':stage.selected_object.origin_src, 'baseimage':stage.image, 'design':canvas.toDataURL('image/png')},
							success: function(res) {
								console.log('success');
							},
        ...
        /*alert("Status: " + textStatus); alert("Error: " + errorThrown); */
						...
						if (
							...
							if (
								...
								var price = (
										...
										(ops.price > 0) ? lumise.func.price(ops.price) : 
										lumise.i(100) : ''
									),
									tags = (
								...
								if (ops.url !== undefined && ops.url.indexOf('dumb-') === 0) {
									lumise.indexed.get(ops.url.split('dumb-')[1], 'dumb', function(res){
										if (res !== null && res !== undefined) {
											...
											lumise.func.notice(lumise.i(165));
										}
									});
									ops.url = '';
								}
					...
					doZoom : function(e){
						...
						[125, 150, 175, 200].map(function(m){
							if (Math.abs(val-m) < 5)
								val = m;
						});
					...
					imageColorFX: function(e) {
						...
						if (this.getAttribute('data-fx') == 'bnw') {
							$('#lumise-image-fx-saturation').val(15).trigger('input');
							lumise.func.update_image_fx('saturation', 15);
						}else if (s.active.fx && s.active.fx.saturation == 15) {
						...
						switch (act) {
							...
							case 'edit' :
								...
								if (id == 'new') {
									...
									if (is_empty_design)
										return lumise.func.notice(lumise.i(96), 'error', 3500);
								...
								} else if ($(e.target).closest('li.active[data-id="'+id+'"]').length > 0) {
									return;
								};
								...
								lumise.indexed.get(id, 'dumb', function(res){
									if (res !== null) {
										...
										lumise.func.edit_design(lumise.ops.my_designs[id]);
									} else {
										lumise.func.notice(lumise.i(166), 'error', 3500);
									}
								});
					...
					fileNav : function(e) {
						...
						switch (func) {
							...
							break;
							case 'saveas' :
								lumise.func.download_design({type: 'json'});
							break;
							case 'save' :
								...
								if (lumise.get.el('saved-designs').find('li[data-view="add"]').length === 0) {
									lumise.get.el('saved-designs').prepend(
							...
							case 'download' :
								...
								lumise.func.download_design({type: this.getAttribute('data-type')});
					...
					doPrint : function(e) {
						...
						if (this.tagName == 'BUTTON') {
							...
							if (format == 'svg' || format == 'pdf')
								return lumise.func.download_design({type: format, include_base: include_base, full: full});
								...
								lumise.func.download_design({
									...
									callback: function(data) {
										...
										if (func == 'download') {
											...
											if (lumise.func.url_var('order_print', '') !== '') {
												name = 'order-'+lumise.func.url_var('order_print')+'__product-'+lumise.func.url_var('product_cms')+'__base-'+lumise.func.url_var('product')+'__stage-'+(Object.keys(lumise.data.stages).indexOf(lumise.current_stage)+1)+'.png';
											};
											lumise.func.download(data, name);
											return;
										};
										...
										html += '<span data-item>\
											...
											<span data-view="func">\
												...
												<span data-view="fsub" data-id="'+s.share_id+'" data-aid="'+s.aid+'" data-link="'+ encodeURIComponent(share_url)+'" data-created="'+s.created+'">\
													<date data-func="date">'+lumise.func.date('h:m D d M, Y', s.updated*1000)+'</date>\
			...
			event_add_text : function(el) {
				...
				['click', function(e){
					...
					if (this.getAttribute('data-act')) {
						lumise.func.preset_import(ops, {}, function() {
			...
			nav : {
				...
				callback : {
					...
					replace : function(el, e) {
						...
						lumise.func.select_image(function(opt) {
							
							lumise.func.replace_image(opt.url, active);
					...
					crop: function(el, e){
						...
						var s = lumise.get.stage(),
							src = s.active.fxOrigin ? s.active.fxOrigin.src :
							...
							save: function(crop) {
								...
								if (s.active) {
									...
									var _e = el.get(0), 
										...
										type = lumise.func.get_type(s.active.src),
										w = s.active.width*(_e.offsetWidth/_c.offsetWidth),
										h = s.active.height*(_e.offsetHeight/_c.offsetHeight);
									
									cv.width = _e.offsetWidth;
									cv.height = _e.offsetHeight;
									
									ctx.drawImage(
										crop.find('img.lumise_crop_img').get(0), 
										-_e.offsetLeft,
										-_e.offsetTop,
										_c.offsetWidth, 
										_c.offsetHeight
									...
									var url = cv.toDataURL('image/'+type);
									
									s.active.setSrc(url, function() {
										s.active.set({
											width: w,
											height: h,
											origin_src: url,
					...
					designs: function(){
						...
						if ($('#lumise-saved-designs p.empty').length > 0)
							lumise.render.refresh_my_designs();
		...
		templates : {
			...
			load : function(tmp, callback) {
				...
				/*sravan modified, instead of =, added >=*/
				if (tmp.upload !== undefined && tmp.upload.indexOf('.lumi') >= -1) {
					...
					/*sravan modified*/
					fabric.Image.fromURL(tmp.upload, function(image) {
				...
				lumise.tools.imports(res, function(stage) {
					...
					if (
						...
						var ez = lumise.stage().limit_zone,
							...
							rt = (ez.width/stage.edit_zone.width),
						...
						lumise.func.scale_designs(scale, {left: left, top: top});
						stage.template.loaded = true;
					...
					if (ops[0].url && ops[0].url.indexOf('dumb-') === 0) {
						lumise.indexed.get(ops[0].url.split('dumb-')[1], 'dumb', function(res){
							if (res !== null & res !== undefined) {
								...
								ops[0].url = res[0];
								lumise.func.preset_import(ops, {});
								delete res;
							} else lumise.func.notice(lumise.i(165));
						});
					} else {
						lumise.func.preset_import(ops, {});
					}
		...
		tools : {
			...
			export : function(stage) {
				console.log('line 5952 export');
				if (!stage || !stage.canvas)
					return null;

				var data = stage.canvas.toJSON(lumise.ops.export_list);
				console.log(data);
				/*console.log(stage.selected_object.origin_src);
        ...
        alert("Status: " + textStatus); alert("Error: " + errorThrown); 
			...
			imports : function(data, callback) {
				...
				if (lumise.data.stages[lumise.current_stage] !== undefined) {
					
					var stage = scale = (lumise.data.stages[lumise.current_stage].data.product_height ? 
								 lumise.data.stages[lumise.current_stage].product.height/
								 lumise.data.stages[lumise.current_stage].data.product_height :
								  1
								);
					...
					this.import(lumise.data.stages[lumise.current_stage].data, function(){
						...
						if (scale !== 1)
							lumise.func.scale_designs(scale);
			...
			save : function(e, id, created){
				...
				lumise.func.export(e == 'designs' ? 'designs' : true, id/*save to db*/, created);
		...
		func : {
			...
			download_design : function (ops) {
				...
				if (lumise.func.url_var('order_print', '') !== '') {
					name = 'order-'+lumise.func.url_var('order_print')+'__product-'+lumise.func.url_var('product_cms')+'__base-'+lumise.func.url_var('product')+'__stage-'+(Object.keys(lumise.data.stages).indexOf(lumise.current_stage)+1);
				}
				...
				switch (type) {
					...
					case 'svg':
						...
						var svg_obj = lumise.func.export_svg(include_base);
						...
						if (svg_obj !== null) {			
							...
							delete svg_obj;
						} else lumise.func.notice('Error on render SVG', 'error');
					...
					case 'png':
						...
						if (typeof ops.callback != 'function') {
							ops.callback = function(data) {
								lumise.func.download(data, name+'.png');
							}
						};
					...
					case 'pdf':
						...
						var stages = Object.keys(lumise.data.stages),
							...
							do_export = function() {
								...
								var exp = lumise.func.export_svg(include_base, true),
									stage = lumise.stage();
						...
						if (stages.filter(function(s) {return lumise.data.stages[s].canvas === undefined;}).length > 0) {
							alert(lumise.i(196));
							return;
						};
			...
			process_files: function(files, callback, saveas) {
				...
				for (f in files) {
					...
					reader[f].addEventListener("load", function () {
						...
						if (!lumise.func.check_upload_size(reader[this.f].file)) {
			...
			select_image: function(callback, saveas) {
				...
				if (ops.image_inp === undefined) {
					...
					ops.image_inp.onchange = function(){
						lumise.func.process_files(this.files, this.callback, this.saveas);
					};
				};
			...
			replace_image: function(url, image) {
				...
				lumise.func.crop({
					...
					save: function(crop) {
						...
						var main_canvas = lumise.stage().canvas,
							active = image,
			...
			check_upload_size : function(file) {
				...
				if (
					...
					if (show_notice) 
						lumise.func.notice(lumise.i('147')+' '+(file.size/1024).toFixed(2)+'KB (Minimum '+lumise.data.min_upload+'KB)', 'error', 8000);
				...
				if (
					...
					if (show_notice) 
						lumise.func.notice(lumise.i('147')+' '+(file.size/1024).toFixed(2)+'KB (Maximum '+lumise.data.max_upload+'KB)', 'error', 8000);
			...
			check_upload_dimensions : function(img) {
				...
				var size = lumise.get.size();
				
				if (lumise.data.min_ppi !== '') {
					
					var pi = parseFloat(lumise.data.min_ppi)/300;
					
				    if (
				    	img.width*pi < size.w ||
				    	img.height*pi < size.h
				    ) {
					    lumise.func.notice(lumise.i(194)+' '+lumise.data.min_ppi, 'notice', 3500);
						return null;
				    }	
			    };
				...
				if (lumise.data.max_ppi !== '') {
					...
					var pi = parseFloat(lumise.data.max_ppi)/300;
					
				    if (
				    	img.width*pi > size.w ||
				    	img.height*pi > size.h
				    ) {
					    lumise.func.notice(lumise.i(195)+' '+lumise.data.max_ppi, 'notice', 3500);
						return null;
				    }	
			    };
			...
			process_variations : function(values, el) {
				...
				var stages = lumise.ops.product_data.stages,
					vari_data = {
					...
					if (
						...
						obj['printings'].map(function(p) {
							if (p['calculate'] && typeof p['calculate'] == 'string')
								p['calculate'] = lumise.func.dejson(p['calculate']);
						});
			...
			keep_current_designs : function(new_stages) {
				...
				var curent_designs = lumise.func.export().stages;
			...
			print_detail : function(id) {
				...
				if (typeof print.calculate == 'string')
					print.calculate = lumise.func.dejson(print.calculate);
			...
			edit_design : function(ops) {
				...
				lumise.tools.save();
				lumise.func.set_url('my_design', ops.id);
				
				lumise.func.load_product({
					id: ops.product,
					cms: ops.product_cms,
					...
					callback: function(res) {
						...
						if (res.id === undefined) {
							lumise.f(false);
							lumise.func.notice('ERROR_LOAD_PRODUCT', 'error', 3500);
							return;
						} else {
				...
				lumise.func.clear_url(['my_design']);
				
				lumise.get.el('saved-designs').find('>li.editing').removeClass('editing');
				lumise.get.el('saved-designs').find('>li[data-id="'+ops.id+'"]').addClass('editing');
				...
				var product = null;
					donow = function(res) {
						...
						if (typeof res.variations == 'string' && res.variations !== '')
							res.variations = lumise.func.dejson(res.variations);
						else res.variations = {};
						
						if (typeof res.attributes == 'string' && res.attributes !== '')
							res.attributes = lumise.func.dejson(res.attributes);
						else res.attributes = {};
						
						if (typeof res.stages == 'string')
							res.stages = lumise.func.dejson(res.stages);
			...
			export : function(save, id, created) {
				...
				if (
		    		...
		    		if (
		    			save == 'designs' || (
		    				lumise.ops.importing !== true && 
		    				lumise.func.url_var('my_design', '') !== ''
		    			)
		    		) {
						
						var design_id = lumise.func.url_var('my_design', ''),
							product_id = lumise.func.url_var('product', '');
							product_cms = lumise.func.url_var('product_cms', '');
						
						if (design_id === '')
							design_id = new Date().getTime().toString(36).toUpperCase();
			...
			scale_designs : function(scale, pos) {
				...
				if (pos !== undefined) {			
					group.set({
						left: group.left-(pos.left !== undefined ? pos.left : 0),
						top: group.top-(pos.top !== undefined ? pos.top : 0)
					});
				} else {
					
					var xl = (stage.limit_zone.width/2)+stage.limit_zone.left,
						yl = (stage.limit_zone.height/2)+stage.limit_zone.top;
			...
			clear_url: function(ex) {
				...
				['car', 'design_print', 'order_print', 'design', 'share', 'my_design'].map(function(i) {
					if (typeof ex !== 'object' || ex.indexOf(i) === -1)
						lumise.func.set_url(i, null);
				});
		...
		render : {
			...
			refresh_my_designs : function(){
				console.log('line no 11233 refresh my designs');
			...
			my_designs : function(design){
				...
				if (lumise.func.version_compare('1.7.1', design.system_version) > 0) {
				...
				design.screenshot = typeof design.screenshot == 'string' ?
									URL.createObjectURL(lumise.func.url2blob(design.screenshot)) : 
				...
				var el = lumise.get.el('saved-designs'),
					cur = lumise.func.url_var('my_design'),
					lis = '';
				
				el.find('p.empty').remove();
				
				if (el.find('>li[data-view="add"]').length === 0) {	
					lis = '<li data-view="add" data-func="edit" data-id="new">\
							<b data-func="edit">+</b>\
							<span data-func="edit">'+lumise.i(107)+'</span>\
						</li>';
				};
				
				lis += '<li data-id="'+design.id+'" class="bgcolorafter'+(cur == design.id ? ' editing' : '')+'">\
							<div data-func="edit" data-view="stages" title="'+lumise.i(48)+'">\
								<span data-func="edit">\
							...
							<em data-view="date">'+lumise.func.date('h:m D d M, Y', design.updated*1000)+'</em>\
			...
			product : function(data, callback, designs) {
				...
				if (data.variations !== undefined && data.variations !== '' && typeof data.variations == 'string')
					data.variations = lumise.func.dejson(data.variations);
				
				if (data.attributes !== undefined && data.attributes !== '' && typeof data.attributes == 'string')
					data.attributes = lumise.func.dejson(data.attributes);
				
				if (data.stages !== undefined && data.stages !== '' && typeof data.stages == 'string')
					data.stages = lumise.func.dejson(data.stages);
				...
				if (Object.keys(data.stages).length === 0) {
					...
					$('#lumise-main').html(
						'<div id="lumise-no-product" style="display: block;">\
							<p>Error, no configuration for the product you choose</p>\
					...
					return lumise.func.notice('Error, no configuration for this product', 'error', 5000);
				}
				...
				var vari_data = lumise.func.process_variations(variations.default, null);
				...
				if (data.saved_stages === undefined) {
					lumise.data.stages = lumise.func.keep_current_designs(vari_data.stages);
				} else {
					var _stages = {};
				...
				lumise.active_stage(lumise.render.stage_nav(), function(){
					...
					else if (lumise.func.url_var('cart', '') !== '') 
					{
			...
			product_attrs : function(data) {
				...
				p.find('header>price').html(lumise.func.price(data.price));
			...
			cart_change : function() {
				...
				if (Object.keys(items).length > 0) {
					Object.keys(items).map(function(id) {
						item = items[id];
						if (lumise.func.version_compare('1.7.1', item.system_version) <= 0) {
							color = '#fefdfe';
							if (
							...
							'<li data-func="edit" data-id="'+id+'">\
								...
								<span data-view="info">\
									'+(id != current_id ? 
										...
										'<name>'+item.name+'</name>'
									)+'<price>'+lumise.func.price(item.price_total)+'</price> \
			...
			cart_details : function(e) {
				...
				if (Object.keys(items).length > 0) {
					Object.keys(items).map(function(id) {
						...
						if (lumise.func.version_compare('1.7.1', items[id].system_version) > 0) {
							delete items[id];
							localStorage.setItem('LUMISE-CART-DATA', JSON.stringify(items));
						...
						Object.keys(item.options).map(function(at) {
							...
							if (
								...
								item.attributes[at].values.split("\n").map(function(v) {
									v = v.split('|');
									if (v[0].trim() == val) {
								...
								val = '<span title="'+item.options[at]+'" style="background: '+item.options[at]+';padding: 3px 5px;border-radius: 2px;color:'+lumise.func.invert(item.options[at])+'">'+val+'</span>';
							}
						...
						setTimeout(lumise.func.cart_thumbn, 100, id);
				...
				if (res.products && res.products.length > 0) {
					...
					res.products.map(function(p) {
						...
						if (p.stages !== '' && typeof p.stages == 'string')
							p.stages = lumise.func.dejson(p.stages);
						
						var stages = lumise.func.pimage(p.stages.stages !== undefined ? p.stages.stages : p.stages);
		...
		active_stage : function(name, callback) {
			...
			if (stage.canvas) {
				...
				lumise.func.stage_nav(name, stage.product.width/stage.product.height);
			...
			fabric.util.loadImage(stage.image, function(img) {
			    ...
			    else 
			    {
					...
					lumise.func.create_canvas(stage, img);
		...
		cart : {
			...
			sum_calc : function (){
				...
				var prices = {},
					...
					price = Object.values(lumise.cart.price).filter(
			...
			add_cart : function(e) {
				...
				var values			= [],
					...
					invalid_fields	= [],
					inv				= null,
					cart_design		= lumise.func.export('cart'),
				...
				*/
				...
				var attrs = $('.lumise-cart-attributes');
				...
				if (invalid_fields.length > 0) {
					...
					lumise.func.notice(lumise.i(179), 'error', 3500);
			...
			variations : function(el) {
				...
				var vari_data = lumise.func.process_variations(values, el);
			...
			calc : function (states_data) {
				...
				$('.lumise-cart-field-quantity input[type="number"]').val(lumise.cart.qty);
			...
			checkout: function(data) {
	            ...
	            Object.keys(data).map(function(key) {
	                ...
	                formData.append(key, new Blob([JSON.stringify(lumise.apply_filters('checkout-item', data[key]))]));
			...
			*/
				...
				lumise.trigger({
					...
					formula : function(e) {
								...
								<div id="lumise-formula-detail">\
									...
									<table>\
										...
										<tr>\
											...
											<td>'+lumise.func.price(sum.ext)+'</td>\
										</tr>\
										<tr>\
											<td style="width:20%;text-align: left">'+lumise.i(74)+'</td>\
											<td><strong>'+lumise.func.price(
												sum.ext+
												sum.base+
												sum.template+
												lumise.cart.printing.calc(lumise.cart.qty)
											)+'</strong> x '+lumise.cart.qty+' qty</td>\
										</tr>\
									</table>\
								</div>\
							</div>'
						...
						$('#lumise-formula-detail a[data-print]').on('click', function(e) {
							e.preventDefault();
							lumise.func.print_detail(this.getAttribute('data-print'));
						});
			...
			fields : {
				...
				render : function(data) {
					...
					var lac = lumise.attributes_cfg[data.type];
					...
					if (typeof data.values != 'object') {
						...
						if (typeof data.values == 'string') {
							...
							var values = data.values.split(decodeURI('%0A'));
							
							if (data.use_variation === true) {
								data.required = true;
								data.values = [{value: '', title: lumise.i(178)}];
							} else data.values = [];
							...
							values.map(function(q) {
								...
								if (
									...
									data.allows.indexOf(q[0].trim()) === -1
								) return;
								
								data.values.push({
									value: q[0].trim(),
									title: (q[1] !== undefined ? q[1].trim() : q[0].trim())
								});
							...
							data.options.map(function(op) {
								data.values.push({
									value: op.quantity !== undefined ? op.quantity : '',
									title: op.title !== undefined ? op.title : ''
								});
							});
						}
					};
					...
					lumise.data.printings.map(function (print, index){
						...
						lumise.trigger({
							...
							price_table : function (e){
								...
								lumise.func.print_detail(this.getAttribute('data-id'));
	        ...
	        do_checkout : function(e) {
		        ...
		        if (lumise.data.conditions !== '') {
			        var content = lumise.func.dejson(lumise.data.conditions).replace(/\>/g, '&gt;').replace(/\</g, '&lt;')+'<em><input type="checkbox" id="condition-agree" /> <label for="condition-agree">'+lumise.i(177)+' <font color="red">*</font></label></em>';
			        lumise.func.confirm({
						title: content,
						primary: {
							text: lumise.i(175),
		...
		load : function() {
			...
			*/
			...
			this.actions.add('ctrl-s', function(e) {
				...
				else return lumise.func.export('designs');
			...
			this.actions.add('cart_edit', function(ops) {
				...
				lumise.func.clear_url();
				lumise.func.set_url('cart', ops.id);
			...
			this.actions.add('db-ready', function(){
				...
				} else if (lumise.func.url_var('my_design', '') !== '') {
					var design_id = lumise.func.url_var('my_design');
					lumise.indexed.get(design_id, 'designs', function(design){
						lumise.indexed.get(design_id, 'dumb', function(res){
							if (design !== null && res !== null) {
								design.stages = res.stages;
								lumise.func.edit_design(design);
							} else {
								lumise.func.notice(lumise.i(166), 'error', 3500);
							}
						});
				...
				} else if (lumise.func.url_var('reorder', '') === '' && lumise.get.el('no-product').length > 0) {
						...
						$.ajax({
							...
							statusCode: {
								...
								200: function(res) {
									...
									lumise.func.clear_url(['design_print', 'order_print']);
												...
												lumise.get.el('general-status').html(
													'<span>\
														<text>\
															<i class="lumisex-android-alert"></i> '+
															lumise.i(192)+
														' <strong>#'+lumise.func.url_var('order_print')+'</strong></text>\
													</span>'
												);
			...
			this.actions.add('product', function(data) {
				...
				*/
				...
				var priacc = lumise.get.el('navigations').find('li[data-tool="print"][data-alwd]'),
					priurl = encodeURIComponent(lumise.func.url_var('design_print', ''));
			...
			this.actions.add('save-design', function(id) {
				console.log('line 15095');
				lumise.get.el('general-status').html(
					'<span>\
						<text>\
							<i class="lumisex-android-alert"></i> '+
							lumise.i(188)+' '+lumise.func.date('h:m - d M, Y', new Date().getTime())+'\
							<strong>#'+id+'</strong>\
						</text>\
						<a href="#cancel-design" data-btn="cancel" data-func="cancel-design">\
							'+lumise.i(187)+'\
				...
				if (lumise.func.url_var('cart', '') === '')
					lumise.func.set_url('my_design', id);
				else lumise.func.set_url('my_design', null);
			...
			this.actions.add('updated', function() {
				if (
					lumise.func.url_var('my_design', '') === '' && 
					lumise.func.url_var('cart', '') === '' &&
					lumise.func.url_var('design_print', '') === ''
				) {
					lumise.get.el('general-status').html(
						'<span>\

...
class lumise_lib{
	...
	public function get_product($id = null) {
		...
		if ($id === null)
	       ...
	       $product = $lumise->db->rawQuery("SELECT id, name, price, product,thumbnail, (SELECT CONCAT( 'http://ec2-18-234-126-164.compute-1.amazonaws.com/',  (SELECT preview from mshop_media WHERE id IN (SELECT refid as id from mshop_product_list where `domain` = 'media'
and parentid = ".$id." and pos =2))))  as 
thumbnail_url , template,
description , stages,variations,`attributes`, printings, `order`,active,created, updated FROM art_products where id=".$id);
				...
				if ($attr->type == 'product_color') {
					...
					if (
						...
						if (!isset($product['variations']->default->{$attr->id}))
							$product['variations']->default->{$attr->id} = '';
					}
				}
